...
// 1. KUPON SORGULA (Müşteri İçin)
router.get('/validate/:code', async (req, res) => {
  try {
    const coupon = await Coupon.findOne({ code: req.params.code.toUpperCase() });
    
    if (!coupon) return res.status(404).json({ message: "Geçersiz kupon kodu." });
    if (!coupon.isActive) return res.status(400).json({ message: "Bu kuponun süresi dolmuş." });

    res.status(200).json(coupon); // İndirim oranını döndür
  } catch (err) {
