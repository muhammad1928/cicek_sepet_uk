      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-1/2 p-8 flex flex-col justify-center">
            ...
            <h1 className="text-4xl font-extrabold text-gray-800 mb-4 mt-2">{product.title}</h1>
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              {product.stock > 0 ? (<span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-bold">Stokta Var</span>) : (<span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-sm font-bold">Stokta Yok</span>)}
            </div>
            ...
            {product.stock > 0 && (
              <div className="flex flex-col gap-4">
                <div className="flex gap-4">
                  ...
                  <button onClick={() => addToCart(product, quantity)} className="flex-1 bg-pink-600 text-white font-bold py-3 rounded-xl hover:bg-pink-700 transition shadow-lg hover:shadow-pink-500/30 active:scale-95">Sepete Ekle</button>
                </div>
                <button onClick={async () => { await toggleFavorite(product._id); fetchProduct(); }} className={`w-full py-3 rounded-xl font-bold border-2 transition flex items-center justify-center gap-2 ${isFav ? "border-red-200 bg-red-50 text-red-600" : "border-gray-200 text-gray-500 hover:border-pink-200 hover:text-pink-500"}`}>
                  <span className="text-xl">{isFav ? "❤️" : "🤍"}</span><span>{isFav ? "Favorilerimden Çıkar" : "Favorilere Ekle"}</span><span className="ml-1 text-xs opacity-70">({product.favoritesCount || 0})</span>
                </button>
              </div>
            )}
          </div>
            ...
            {user ? (
              <form onSubmit={submitReview} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div className="mb-4">
                  ...
                  <div className="flex gap-2">{[1, 2, 3, 4, 5].map(star => (<button key={star} type="button" onClick={() => setRating(star)} className={`text-3xl transition ${star <= rating ? "text-yellow-400 scale-110" : "text-gray-200"}`}>★</button>))}</div>
                </div>
                <div className="mb-4"><label className="block text-sm font-bold text-gray-700 mb-2">Yorumunuz</label><textarea value={reviewText} onChange={(e) => setReviewText(e.target.value)} className="w-full p-4 border rounded-xl focus:ring-2 focus:ring-pink-500 outline-none h-32 bg-gray-50" placeholder="Ürün hakkında ne düşünüyorsunuz?" /></div>
              ...
              <div className="bg-blue-50 p-6 rounded-xl text-blue-800 border border-blue-100 text-center"><p>Yorum yapmak için <span className="font-bold cursor-pointer underline" onClick={() => navigate("/login")}>Giriş Yapmalısınız</span>.</p></div>
            )}
          </div>

          {/* Yorum Listesi (OYLAMA YOK) */}
          <div>
            <h3 className="text-2xl font-bold text-gray-800 mb-6">Yorumlar ({product.reviews.length})</h3>
            <div className="space-y-4">
              {product.reviews.length === 0 ? <p className="text-gray-400 italic">Henüz yorum yapılmamış.</p> : 
                product.reviews.slice().reverse().map((review, index) => (
                  <div key={index} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-start mb-2">
                      <div><span className="font-bold text-gray-800 block">{review.user}</span><span className="text-xs text-gray-400">{new Date(review.date).toLocaleDateString()}</span></div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans pt-24 pb-10 px-4">
      ...
      {product && (
        <Seo 
          title={product.title} 
          description={`${product.title} - £${product.price}. ${product.desc.substring(0, 120)}...`} 
          keywords={`${product.category}, çiçek siparişi, ${product.title}`}
        />
      )}
      <h3 className="text-2xl font-bold text-gray-800 mb-6">Bunları da Beğenebilirsiniz</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {related.map((item) => (
          <div key={item._id} className="bg-white rounded-xl border border-gray-100 overflow-hidden cursor-pointer hover:shadow-lg transition group" onClick={() => { navigate(`/product/${item._id}`); window.scrollTo(0, 0); }}>
            <div className="h-40 overflow-hidden relative"><img src={item.img} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" /></div>
            <div className="p-3"><h4 className="font-bold text-gray-800 text-sm truncate">{item.title}</h4><div className="text-pink-600 font-bold mt-1">£{item.price}</div></div>
          </div>
        ))}
      </div>
    </div>
