    ...
    <div className="fixed inset-0 z-[999] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-in z-20">
        ...
        <div className="p-5 border-b flex justify-between items-center bg-gray-50">
          <div className="flex items-center gap-3"><h2 className="text-xl font-bold text-gray-800">Sepet ({cart.length})</h2>{cart.length > 0 && <button onClick={() => setShowClearConfirm(true)} className="text-xs font-bold text-red-500 bg-red-50 hover:bg-red-100 px-2 py-1 rounded transition">Temizle</button>}</div>
              ...
              <div key={item._id} className="flex gap-4 border-b pb-4 items-center">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-0 mt-2 bg-gray-100 w-fit rounded-lg border border-gray-200">
                    ...
                    <input type="number" value={item.quantity} onChange={(e) => handleQuantityInput(e, item)} className="w-10 h-8 text-center font-bold text-gray-800 bg-transparent outline-none text-sm appearance-none m-0" />
                ...
                <button onClick={() => handleDeleteClick(item)} className="text-2xl hover:scale-110 transition p-2" title="Sil">🌸</button>
              </div>
            ))
          }
        </div>
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            {!couponApplied ? (
              <div className="flex gap-2">
                <input type="text" placeholder="İndirim Kodu" className="flex-1 border border-gray-300 rounded p-2 text-sm uppercase outline-none focus:border-pink-500" value={couponCode} onChange={(e) => setCouponCode(e.target.value.toUpperCase())} />
                <button onClick={handleApplyCoupon} className="bg-orange-500 text-white px-4 py-2 rounded text-sm font-bold hover:bg-orange-600 transition shadow-md">Uygula</button>
              </div>
            ) : (
              <div className="flex justify-between items-center bg-green-100 p-2 rounded border border-green-200 text-green-700 text-sm font-bold"><span>🎟️ {couponCode} uygulandı (-%{discount})</span><button onClick={() => {setDiscount(0); setCouponApplied(false); setCouponCode("");}} className="text-red-500 hover:underline">Kaldır</button></div>
            )}
          </div>
        ...
        {showCheckoutForm && (
          <div className="absolute inset-0 bg-white z-30 flex flex-col animate-fade-in">
            <div className="p-4 border-b flex justify-between items-center bg-pink-600 text-white"><h2 className="text-lg font-bold">Teslimat Bilgileri</h2><button onClick={() => setShowCheckoutForm(false)} className="text-white/80 hover:text-white font-bold text-sm">KAPAT</button></div>
            <form onSubmit={proceedToPayment} className="flex-1 p-6 overflow-y-auto bg-gray-50 space-y-6">
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <input name="senderPhone" value={formData.senderPhone} onChange={handleInputChange} className={inputClass("senderPhone")} placeholder="Telefonunuz *" />
                  <input name="senderEmail" value={formData.senderEmail} onChange={handleInputChange} className="p-2 border rounded w-full col-span-2 focus:border-pink-500 outline-none" placeholder="E-posta Adresiniz" />
                </div>
              </div>
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                <div className="flex justify-between items-center mb-3 border-b pb-1"><h3 className="text-sm font-bold text-pink-600 uppercase">2. Alıcı Bilgileri</h3>{savedAddresses.length > 0 && (<select onChange={handleAddressSelect} className="text-xs border border-pink-300 rounded p-1 text-gray-600 bg-pink-50 outline-none max-w-[150px]"><option value="">Kayıtlı Adres Seç...</option>{savedAddresses.map((addr, i) => (<option key={i} value={i}>{addr.title}</option>))}</select>)}</div>
                <div className="grid grid-cols-2 gap-3">
                  <input name="recipientName" value={formData.recipientName} onChange={handleInputChange} className={`col-span-2 ${inputClass("recipientName")}`} placeholder="Alıcı Adı Soyadı *" />
                  <input name="recipientPhone" value={formData.recipientPhone} onChange={handleInputChange} className={`col-span-2 ${inputClass("recipientPhone")}`} placeholder="Alıcı Telefonu *" />
                  <textarea name="address" value={formData.address} onChange={handleInputChange} className={`col-span-2 h-20 ${inputClass("address")}`} placeholder="Tam Adres *" />
                  <input name="city" value={formData.city} onChange={handleInputChange} className={inputClass("city")} placeholder="Şehir *" />
                  <input name="postcode" value={formData.postcode} onChange={handleInputChange} className="p-2 border rounded w-full focus:border-pink-500 outline-none" placeholder="Posta Kodu" />
                </div>
              </div>
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-xs text-gray-500 font-bold">Tarih *</label><input type="date" name="deliveryDate" min={new Date().toISOString().split("T")[0]} value={formData.deliveryDate} onChange={handleInputChange} className={inputClass("deliveryDate")} /></div>
                    <div><label className="text-xs text-gray-500 font-bold">Saat Aralığı</label><select name="timeSlot" value={formData.timeSlot} onChange={handleInputChange} className="p-2 border rounded w-full bg-white focus:border-pink-500 outline-none"><option>09:00 - 12:00</option><option>12:00 - 18:00</option><option>18:00 - 22:00</option></select></div>
                  </div>
                  <textarea name="cardMessage" value={formData.cardMessage} onChange={handleInputChange} className="p-2 border rounded w-full h-16 text-sm focus:border-pink-500 outline-none" placeholder="Kart Notunuz..." />
                  ...
                  <input name="courierNote" value={formData.courierNote} onChange={handleInputChange} className="p-2 border rounded w-full text-xs focus:border-pink-500 outline-none" placeholder="Kuryeye Not" />
                </div>
              </div>
              <div className="pt-2 pb-10"><div className="flex justify-between text-lg font-bold text-gray-800 mb-4 px-2"><span>Ödenecek Tutar:</span><span className="text-pink-600">£{finalPrice.toFixed(2)}</span></div><button type="submit" className="w-full bg-green-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-green-700 transition shadow-lg shadow-green-500/30">Öde ve Tamamla 💳</button></div>
            </form>
          </div>
        )}
