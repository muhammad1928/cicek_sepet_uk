    ...
    >
      ...
      <div className="p-6 flex flex-col lg:flex-row items-center justify-between gap-6" onClick={onToggle}>
        ...
        <div className="flex items-center gap-5 w-full lg:w-auto">
          ...
          <div>
            <div className="font-mono text-xs text-gray-400 font-bold flex items-center gap-2 mb-1">
              ...
              {order.status === "İptal Talebi" && <span className="text-orange-600 bg-orange-100 px-2 py-0.5 rounded text-[10px] flex items-center gap-1 font-bold border border-orange-200"><FiAlertCircle/> TALEP</span>}
              {isLoss && <span className="text-red-600 bg-red-50 px-2 py-0.5 rounded text-[10px] flex items-center gap-1 font-bold border border-red-100" title={`Zarar: £${lossAmount.toFixed(2)}`}><FiTrendingDown/> ZARAR</span>}
            </div>
        ...
        <div className="hidden md:flex flex-col items-center">
            <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Satıcı / Mağaza</div>
        ...
        <div className="flex items-center gap-6 w-full lg:w-auto justify-between lg:justify-end">
          <div className="text-right">
            ...
            {isLoss && <div className="text-[10px] text-red-400 font-bold line-through">Maliyet: £{realCost.toFixed(2)}</div>}
          </div>
          ...
          <div onClick={(e) => e.stopPropagation()}>
            ...
            >
              <option>Sipariş Alındı</option><option>Hazırlanıyor</option><option>Hazır</option><option>Yola Çıktı</option><option>Teslim Edildi</option><option className="bg-orange-100 text-orange-800 font-bold">İptal Talebi</option><option className="text-red-600 font-bold">İptal</option>
            </select>
          </div>
        </div>
      </div>
        ...
        <div className="border-t border-gray-100 bg-gray-50/50 p-6 lg:p-8 cursor-default animate-fade-in" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-white p-5 rounded-2xl border border-gray-200 text-sm shadow-sm">
                ...
                <p className="text-xs text-gray-400 uppercase font-bold mb-1">Gönderen</p>
            ...
            {/* 3. Notlar & Finans */}
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-gray-400 uppercase flex items-center gap-2">📝 Notlar & Finans</h4>
              <div className="flex flex-col gap-3 h-full">
                  {order.delivery.cardMessage ? <div className="bg-pink-50 p-4 rounded-2xl border border-pink-100 text-sm text-pink-800 italic shadow-sm flex-1"><span className="text-2xl opacity-20 block">❝</span>{order.delivery.cardMessage}</div> : <div className="text-xs text-gray-400 italic p-4 bg-white rounded-2xl border border-dashed border-gray-300">Kart notu yok.</div>}
                  ...
                  {isLoss && (
                      <div className="bg-red-50 p-3 rounded-xl border border-red-100 text-xs text-red-700 font-mono mt-auto">
                        <div className="flex justify-between"><span>Gerçek Maliyet:</span> <span>£{realCost.toFixed(2)}</span></div>
                        <div className="flex justify-between font-bold border-t border-red-200 pt-1 mt-1"><span>NET ZARAR:</span> <span>-£{lossAmount.toFixed(2)}</span></div>
                      </div>
                  )}
                  {order.status === "İptal Talebi" && order.cancellationReason && (
                    <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg animate-pulse">
                      <div className="flex items-center gap-2 text-red-800 font-bold mb-1"><FiAlertCircle /><span>İPTAL SEBEBİ:</span></div>
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
            <button onClick={(e) => { e.stopPropagation(); setShowMetaData(!showMetaData); }} className="text-xs font-bold flex items-center gap-2 transition px-5 py-3 rounded-xl border bg-white text-slate-500 border-slate-200 hover:border-slate-400">
              {showMetaData ? <FiEyeOff /> : <FiEye />} {showMetaData ? "Teknik Verileri Gizle" : "Güvenlik & Metadata Göster"}
            </button>

            <div className="flex items-center gap-4">
               {order.courierId && <span className="text-xs bg-purple-100 text-purple-700 px-4 py-2 rounded-xl font-bold border border-purple-200 flex items-center gap-2"><FiTruck/> Kurye Atandı</span>}
               <button onClick={() => onInvoiceClick(order)} className="flex items-center gap-2 bg-gray-900 text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-black transition shadow-lg transform active:scale-95"><FiPrinter /> Fatura Yazdır</button>
            </div>
          </div>
