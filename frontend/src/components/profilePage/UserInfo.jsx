...
const UserInfo = ({ user }) => {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      notify(t("profilePage.userInfo.profileUpdated") + " ✨", "success");
      setIsEditing(false);
    } catch (err) {
      notify(t("profilePage.userInfo.updateUnsucessfull"), "error");
    }
  };
  ...
  const handlePasswordUpdate = async (e) => {
    e.preventDefault();
    if (!passwords.oldPass) return notify(t("profilePage.userInfo.oldPassword"), "warning");
    if (!passwordValid) return notify(t("profilePage.userInfo.passwordInvalid"), "error");
    if (!isMatch) return notify(t("profilePage.userInfo.passwordsNotMatch"), "error");
    ...
    try {
      ...
      notify(t("profilePage.userInfo.passwordChanged"), "success");
      localStorage.removeItem("user");
      window.dispatchEvent(new Event("user-change"));
      setTimeout(() => window.location.href="/login", 2000);
    } catch (err) {
      notify(err.response?.data?.message || t("profilePage.userInfo.passwordChangeFailed"), "error");
    }
  };
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-gray-100 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-8">
           <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-extrabold text-gray-800">{t("profilePage.userInfo.personalInfo")}</h2>
                <p className="text-xs text-gray-500 font-medium">{t("profilePage.userInfo.personalContactInfo")}</p>
              </div>
           </div>
           {!isEditing ? (
             <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 text-sm font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 px-4 py-2 rounded-xl transition">
               <FiEdit2 /> {t("profilePage.userInfo.editProfile")}
             </button>
           ) : (
             <button onClick={() => setIsEditing(false)} className="flex items-center gap-2 text-sm font-bold text-gray-500 hover:bg-gray-100 px-4 py-2 rounded-xl transition">
               <FiX /> {t("profilePage.userInfo.cancel")}
             </button>
           )}
        </div>
        ...
        <form onSubmit={handleProfileUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className="space-y-2">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{t("profilePage.userInfo.fullName")}</label>
           ...
           <div className="space-y-2">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{t("profilePage.userInfo.email")}</label>
              <div className="relative group">
                ...
                />
                <span className="absolute right-4 top-4 text-[10px] font-bold bg-green-100 text-green-600 px-2 py-0.5 rounded">{t("profilePage.userInfo.verified")}</span>
              </div>
           </div>

           {/* Telefon (Yeni Alan) */}
           <div className="space-y-2">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{t("profilePage.userInfo.phone")}</label>
              <div className="relative">
           ...
           {/* Rol (Badge) */}
           <div className="space-y-2">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{t("profilePage.userInfo.accountType")}</label>
              <div className="relative">
           ...
           {isEditing && (
             <div className="md:col-span-2 flex justify-end mt-2">
               <button type="submit" className="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-blue-500/30 hover:scale-105 transition transform flex items-center gap-2">
                 <FiSave /> {t("profilePage.userInfo.saveChanges")}
               </button>
             </div>
           )}
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-gray-100 relative overflow-hidden">
         ...
         <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-extrabold text-gray-800">{t("profilePage.userInfo.passwordSecurity")}</h2>
                  <p className="text-xs text-gray-500 font-medium">{t("profilePage.userInfo.protectAccount")}</p>
                </div>
            </div>
            {!showPasswordForm && (
              <button onClick={() => setShowPasswordForm(true)} className="text-sm font-bold text-purple-600 bg-purple-50 hover:bg-purple-100 px-5 py-2.5 rounded-xl transition border border-purple-200">
                {t("profilePage.userInfo.changePassword")}
              </button>
            )}
         </div>
         ...
         {showPasswordForm && (
           <form onSubmit={handlePasswordUpdate} className="bg-gray-50/50 p-6 rounded-3xl border border-gray-100 animate-fade-in-down space-y-5">
              ...
              <div className="flex justify-between items-center mb-2">
                 <h4 className="font-bold text-gray-700">{t("profilePage.userInfo.createNewPassword")}</h4>
                 <button type="button" onClick={() => setShowPasswordForm(false)} className="text-gray-400 hover:text-red-500"><FiX size={20}/></button>
              </div>

              <div className="space-y-4">
                <div className="relative">
                  <input type={showOldPass ? "text" : "password"} placeholder={t("profilePage.userInfo.currentPassword")} required className={`${modernInputClass} bg-white`} onChange={e => setPasswords({...passwords, oldPass: e.target.value})} />
                ...
                <div className="relative">
                  <input type={showNewPass ? "text" : "password"} placeholder={t("profilePage.userInfo.newPassword")} required minLength={8} className={`${modernInputClass} bg-white`} onChange={handleNewPassChange} onFocus={()=>setPasswordFocused(true)} onBlur={()=>setPasswordFocused(false)} />
                ...
                {!isMatch && passwords.confirmPass && <p className="text-red-500 text-xs font-bold animate-pulse pl-1">⚠️ {t("profilePage.userInfo.passwordsNotMatch")}</p>}
                ...
                <button type="submit" disabled={!passwordValid || !isMatch} className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg transition transform active:scale-95 ${(!passwordValid || !isMatch) ? 'bg-gray-300 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600 hover:shadow-purple-500/30'}`}>
                  {t("profilePage.userInfo.updatePassword")}
                </button>
              </div>
           </form>
         )}
