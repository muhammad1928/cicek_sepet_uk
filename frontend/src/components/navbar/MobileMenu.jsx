...
import { FiSearch, FiGlobe, FiHeart, FiLogOut, FiChevronDown } from "react-icons/fi";
...
import { publicRequest } from "../../../requestMethods"; // Yol güncellendi
          ...
          {searchTerm.length > 1 && suggestions.length > 0 && (
            <div className="mt-2 bg-white rounded-xl border border-gray-100 shadow-sm">
              {suggestions.map((product) => (
                <div key={`m-${product._id}`} onClick={() => { navigate(`/product/${product._id}`); setIsOpen(false); setSearchTerm(""); }} className="flex items-center gap-3 p-3 border-b border-gray-50 last:border-none">
